...
$template = HTML::Template->new(filename => 'tzauth.htm', die_on_bad_params=>0 );
$template->param( SNAME=> $sname  );
$title="iPasolink time/timezone set tool";
...
$Param->{tzmanualallow}=$Param->{tzmanualallow}?1:0;
...
if(  Action() ==0 ) {
	...
	$template->param( TZMANUAL=> $Param->{tzmanual} );
	$template->param( TZMANUALALLOW=> $Param->{tzmanualallow} );
