...
$template = HTML::Template->new(filename => 'erpread.htm', die_on_bad_params=>0 );
$template->param( SNAME=> $sname  );
$title="iPasolink ERP report tool";
...
$template->param( ERPIDSTART=> $Param->{erpidstart} );
$template->param( ERPIDSTOP=> $Param->{erpidstop} );
$template->param( ALLERP=> $Param->{allerp} );
$template->param( ERPOFF=> $Param->{erpoff} );
...
sub check_record {
	my $retval=1;
	unless( $Param->{allerp} ) {
		unless( CheckField ( $Param->{erpidstart} ,'int', "Field 'start ERP' " )) {
			$retval=0;
		} 
		unless( CheckField ( $Param->{erpidstop} ,'int', "Field 'stop ERP' " )) {
			$retval=0;
		} 
	}
