...
$template = HTML::Template->new(filename => 'vlanread.htm', die_on_bad_params=>0 );
$template->param( SNAME=> $sname  );
$title="iPasolink VLAN report tool";
...
$template->param( PORTASSIGNMENT=> $Param->{portassignment} );
$template->param( VLANIDSTART=> $Param->{vlanidstart} );
$template->param( VLANIDSTOP=> $Param->{vlanidstop} );
$template->param( ALLVLAN=> $Param->{allvlan} );
...
sub check_record {
	...
	unless( $Param->{allvlan} ) {
		unless( CheckField ( $Param->{vlanidstart} ,'int', "Field 'VLAN start' " )) {
				$retval=0;
		} 
		if( $Param->{vlanidstop} ) {
			unless( CheckField ( $Param->{vlanidstop} ,'int', "Field 'VLAN stop' " )) {
				$retval=0;
			} 
			if(  $Param->{vlanidstop} < $Param->{vlanidstart} ) {
				message2( "'VLAN stop' must be equiv or greater than 'VLAN start'" );
				$retval=0;
			}
		}
	}
