...
my @AA=qw( 
portassignment vlanidstart vlanidstop allvlan
) ;
my $export_param=join( ' ', map{ "$_='$ip_param->{$_}' " } @AA )  ;

if( $ip_param->{portassignment} ) {
	WriteFile( $outfile, "NE name,NE IP,VLAN ID,VLAN name,Assignment,Slot,Port,Mode\n" ) ;
} else {
	WriteFile( $outfile, "NE name,NE IP,VLAN ID,VLAN name\n" ) ;
}
######### header of worker output table 
