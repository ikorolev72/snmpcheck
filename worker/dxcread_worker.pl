...
WriteFile( $outfile, "NE name,NE type,IP,Result,Service ID,Service name,Type,Item A,Slot A,Port A,Channel A,Port type A,Port name A,Item B,Slot B,Port B,Channel B,Port type B,Port name B,Protection A,Item A prot,Slot A prot,Port A prot,Channel A prot,Port type A prot,Port name A prot,Protection B,Item B prot,Slot B prot,Port B prot,Channel B prot,Port type B prot,Port name B prot\n" ) ;
######### header of worker output table 
