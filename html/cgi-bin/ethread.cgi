...
$sname="ethread";
$title="iPasolink Ethernet interfaces report tool";
$ENV{ "HTML_TEMPLATE_ROOT" }=$Paths->{TEMPLATE};
$template = HTML::Template->new(filename => 'ethread.htm', die_on_bad_params=>0 );
...
$template->param( ADMINSTATUS=> $Param->{adminstatus} );
...
sub check_record {
	my $retval=1;
	unless( CheckField ( $Param->{adminstatus} ,'login', "Field 'Admin status' " )) {
			$retval=0;
	} 	
